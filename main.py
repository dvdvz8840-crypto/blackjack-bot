...
# Активные фармы: user_id -> {"count": int, "next_available": timestamp, "timers": []}
active_farms = {}

# Награды за фарм
farm_rewards = {1: 100, 2: 250}

@bot.message_handler(func=lambda m: m.text.lower().startswith("бфарм"))
def start_farm(message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        bot.send_message(message.chat.id, "❗ Используй: бфарм <1 или 2> час [кол-во повторов (макс 3)]")
        return

    try:
        hours = int(parts[1])
        if hours not in [1, 2]:
            bot.send_message(message.chat.id, "❌ Можно фармить только на 1 или 2 часа.")
            return
    except:
        bot.send_message(message.chat.id, "❌ Некорректное время фарма. Введите 1 или 2.")
        return
    ...
    if repeats < 1 or repeats > 3:
        bot.send_message(message.chat.id, "❌ Количество повторов должно быть от 1 до 3.")
        return

    now = int(time.time())
    if user_id in active_farms:
        farm_data = active_farms[user_id]
        if farm_data["count"] >= 3 and now < farm_data["next_available"]:
            remaining = farm_data["next_available"] - now
            hours_rem = remaining // 3600
            minutes_rem = (remaining % 3600) // 60
            bot.send_message(message.chat.id,
                             f"⚠ Вы достигли лимита фарма на сегодня. Доступно через {hours_rem}ч {minutes_rem}м.")
            return
    else:
        active_farms[user_id] = {"count": 0, "next_available": 0, "timers": []}

    bot.send_message(message.chat.id,
                     f"✅ Фарм активирован: {hours}-часовой цикл x{repeats}. После каждого цикла вам будут начислены монеты.")

    # Функция начисления монет
    def farm_tick():
        reward = farm_rewards[hours]
        balance = get_balance(user_id)
        update_balance(user_id, balance + reward)
        # Имя с ссылкой на профиль
        if username:
            name_link = f'<a href="https://t.me/{username}">{first_name}</a>'
        else:
            name_link = first_name
        bot.send_message(message.chat.id,
                         f"✅ {name_link}, вам начислено {reward} монет за фарм!",
                         parse_mode="HTML")
    ...
    def run_farm(index):
        ...
        farm_tick()
        active_farms[user_id]["count"] += 1
        if active_farms[user_id]["count"] >= 3:
            # Блокировка фарма на день
            active_farms[user_id]["next_available"] = int(time.time()) + 86400

    # Создаем потоки для каждого повтора
    for i in range(repeats):
        t = threading.Thread(target=run_farm, args=(i,))
        t.start()
        active_farms[user_id]["timers"].append(t)
# ---------------- Запуск ----------------
